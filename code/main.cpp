...
int main(int argc, char* argv[]) {
    ...
    // SolutionGraph object
    SolutionGraph G;

    // Check if the model is MODELOMILP
    ...
    // Check if the model is MODELOCP
    ...
    // Check if the metaheuristic is TAYEBI
    ...
    // Read additional parameters for local search and metaheuristics
    const char
    ...
    // Read tolerancy and critical operations parameters
    ...
    // Read iteration parameters for metaheuristics
    if (strcmp(metaHeuristic, "None") != 0)
        itMax = stoi(argv[12 * 2]);

    // Read specific parameters for ILS, SA, TS, and GRASP metaheuristics
    ...
    // Apply constructive heuristics and local search if no metaheuristic is selected
    if (strcmp(metaHeuristic, "None") == 0) {
